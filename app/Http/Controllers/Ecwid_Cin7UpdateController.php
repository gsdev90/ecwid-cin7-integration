...
class Ecwid_Cin7UpdateController extends Controller
{
    ...
    public function createCustomerForCin7($order)
    {
        ...
        try {
            ...
            // Generate detailed sale order
            $this->generateSaleOrder($saleOrder['ID'], $order);
    ...
    public function generateSaleOrder($saleId, $order)
    {
        Log::info('Updating Sale Order:', ['saleId' => $saleId]);
        ...
        $data = [
            ...
            "Lines" => [
                [
                    "ProductID" => "d4ab91df-dd63-4088-81b0-4c61640dc673",
                    "SKU" => "HTR-JIF-0609",
                    "Name" => "JIC Female Field Fit Reusable Hydraulic Hose Fittings (Straight)",
                    "Quantity" => 1,
                    "Price" => 8,
                    "Discount" => 0,
                    "Tax" => 0,
        ...
        try {
            ...
            $body = $response->getBody();
            $content = json_decode($body->getContents(), true);
            Log::info('Sale Order Updated: ', $content);
